#include "../interface/explore_map.h"
...
extern void mMove (ExploreMap *obj, int x, int y) {
    obj->currX = (int) x / obj->mapResolutionCM + obj->initX;
    obj->currY = (int) y / obj->mapResolutionCM + obj->initY;
}

/* Add the data collected from the sensor */
extern void mAddData (ExploreMap *obj, int y_neg, int x_pos, int y_pos, int x_neg) {
    unsigned int nEmpty;
    int i;
...
extern MapExplorationDir mGetBestDir (ExploreMap *obj, MapExplorationDir currDir) {
    ...
    **/
    ...
    int currSum = 0;
    if (currDir == Y_NEG) { currSum = y_neg_sum; }
    if (currDir == X_POS) { currSum = x_pos_sum; }
...
extern void mBuildFlowMap(ExploreMap *obj) {
    ...
    if(obj->currX > -1 && obj->currX < MAP_SIZE && obj->currY > -1 && obj->currY < MAP_SIZE) {
        ...
        while(stop == 'n') {
            int newNodeCount = 0;
            for (unsigned int i = 0; i < obj->newNodes->used; i++) {
